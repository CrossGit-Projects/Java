...
public class AppUserValidator implements Validator {
    ...
    public static final String TELEPHONE_PATTERN = "^\\s?((\\+[1-9]{1,4}[ \\-]*)|(\\([0-9]{2,3}\\)[ \\-]*)|([0-9]{2,4})[ \\-]*)*?[0-9]{3,4}?[ \\-]*[0-9]{3,4}?\\s?";
    ...
    public void validate(Object arg0, Errors errors) {
        ...
        if (errors.getErrorCount() == 0) {
            if (StringUtils.hasText(((AppUser)arg0).getEmail()) && emailValidator.isValid(((AppUser)arg0).getEmail()) == false) {
             ...
             if(!this.matchesNumber(((AppUser) arg0).getTelephone())){
                    errors.rejectValue("telephone", "error.email.phone");
             }

        }
    }

    private static boolean matchesNumber(String phoneNumber){
        return phoneNumber.matches(TELEPHONE_PATTERN);
    }
